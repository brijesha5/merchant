  ...
  Future<List<Map<String, dynamic>>> fetchOnlineItems() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Debug: Log the entire pricing data
        ...
        List<Map<String, dynamic>> filteredItems = data
            ...
            .map((json) => {
          "id": json["id"] ?? 0,
          "itemName": json["itemName"] ?? "Unknown Itemsss",  // Make sure to use the correct key here.
    ...
    try {
      final response = await http.post(
        ...
        body: json.encode({
          "merchant_id": merchantId,
          "status": status,
          "item_ids": itemIds, // Send all item IDs in a list
...
////
  Future<void> fetchOnlineOrders() async {
    try {
      ...
      print("API Response for Online Orders: ${response.body}");
...
/////
                              ...
                              child: Column(
                                children: [
                                  ListTile(
                                    ...
                                    subtitle: Text(
                                      "$formattedTime | $items | ₹${order.grossAmount.toStringAsFixed(2)}",
  ...
  async {
    DateTime fromTime = DateTime.now(); // Fixed current time
    DateTime toTime = fromTime; // Initialize to same time, user will change it
...
Future<void> exportToExcel(List<Map<String, dynamic>> data, String fileName) async {
  ...
  if (await Permission.storage.request().isGranted) {
    ...
    if (Platform.isAndroid) {
      ...
      // Handle other platforms like web, macOS, Windows, Linux if needed
      directory = await getApplicationDocumentsDirectory();
    }
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: ConstrainedBox(
                        constraints: BoxConstraints(
                          minWidth: MediaQuery.of(context).size.width - 200, // Ensures full-width header
                        ),
                        child: DataTable(
                          headingRowColor: MaterialStateColor.resolveWith((states) => Colors.black),
                          headingTextStyle: const TextStyle(color: Colors.white),
                          columnSpacing: 100.0, // Reduced spacing but still maintains readability
                          columns: const [
                            ...
                            DataColumn(
                              label: Expanded(
                                child: Align(
                                  alignment: Alignment.centerLeft, // Moves Total Amount text slightly left
                          ...
                          rows: itemWiseData.map((row) {
                            return DataRow(cells: [
                              ...
                              DataCell(Align(
                                alignment: Alignment.centerLeft, // Moves text inside cell slightly left
                  ...
                  // The Download Button below the Data Table
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: ConstrainedBox(
                        ...
                        child: DataTable(
                          headingRowColor: MaterialStateColor.resolveWith((states) => Colors.black),
                          headingTextStyle: const TextStyle(color: Colors.white),
                          columnSpacing: 120.0, // Reduced spacing for better alignment
                          columns: const [
                            ...
                            DataColumn(
                              label: Expanded(
                                child: Align(
                                  alignment: Alignment.centerLeft, // Moves "Total Amount" slightly left
                                  child: Text('Total Amount'),
                                ),
                              ),
                            ),
                          ...
                          rows: canceledOrdersData.map((row) {
                            return DataRow(cells: [
                              ...
                              DataCell(Align(
                                alignment: Alignment.centerLeft, // Moves "Total Amount" inside cell slightly left
                                child: Text(row['totalAmount'].toString() ?? '0.00'),
                              )),
                            ]);
                          }).toList(),
