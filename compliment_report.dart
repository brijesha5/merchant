...
void main() {
  runApp(const ComplimentaryReport());
}

class ComplimentaryReport extends StatelessWidget {
  const ComplimentaryReport({Key? key}) : super(key: key);
    ...
    return MaterialApp(
      ...
      home: WillPopScope(
        ...
        child: Scaffold(
          ...
          body: const ComplimentaryReportDataPage(),
        ),
      ),
    );
  }
...
class ComplimentaryReportDataPage extends StatefulWidget {
  const ComplimentaryReportDataPage({Key? key}) : super(key: key);

  @override
  _ComplimentaryReportDataPageState createState() =>
      _ComplimentaryReportDataPageState();
}

class _ComplimentaryReportDataPageState
    extends State<ComplimentaryReportDataPage> {
  List<dynamic> data = [];
  bool isLoading = true;
  String errorMessage = '';
  DateTime? startDate;
  ...
  void initState() {
    super.initState();
    // Set the current date as the default start and end date
  ...
  Future<void> fetchData() async {
    ...
    final response = await http.get(Uri.parse('${apiUrl}report/compliment?DB=$CLIENTCODE&startDate=$start&endDate=$end'));
  ...
  Future<String> exportToPdf(BuildContext context) async {
    ...
    pdf.addPage(
      pw.Page(
        build: (pw.Context context) {
          return pw.Column(
            children: [
              pw.Text('Complimentary Report', style: pw.TextStyle(fontSize: 24)),
              pw.SizedBox(height: 16),
              pw.Table.fromTextArray(
                headers: ['Bill No', 'Bill Date', 'Bill Amount', 'Remark', 'Waiter'],
                data: data.map((item) {
                  return [
                    item['billNo'] ?? 'N/A',
                    item['billDate'] ?? 'N/A',
    ...
    final file = File("${output.path}/complimentary_report.pdf");
  ...
  Future<File> generateExcel(List<dynamic> data) async {
    ...
    // Add header row
    sheetObject.appendRow(['Bill No', 'Bill Date', 'Bill Amount', 'Remark', 'Waiter']);
    ...
    final path = '${directory.path}/complimentary_data.xlsx';
              ...
              child: DataTable(
                ...
                rows: data.map((item) {
                  return DataRow(cells: [
                    ...
                    DataCell(Text(item['billAmount'] ?? 'N/A')),
          ...
          children: [
            FloatingActionButton.extended(
              onPressed: () async {
                ...
                await Printing.sharePdf(bytes: await File(filePath).readAsBytes(), filename: 'complimentary_report.pdf');
              },
